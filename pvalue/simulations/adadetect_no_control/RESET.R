...
for (j in 1:length(alphas)) {
  ...
  discs = (res$q_vals <= alpha & res$Labels == 1)
  false_disc <- sum(discs & H0 == 0)
  true_disc <- sum(discs & H0 == 1)
...
df_out1 = data.frame(FDP = summary_FDP, power = summary_power, type = rep('reset_ensemble', length(alphas)), flag = flag, alpha = alphas)
