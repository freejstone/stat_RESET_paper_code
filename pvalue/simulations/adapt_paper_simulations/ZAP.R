...
for (file in files) {
  print(file)
  df = read.csv(paste('data1', file, sep = '/'))
  x = df[, grepl('x', colnames(df))]
  x = cbind(splines::ns(x[, 1], df = 6), splines::ns(x[, 2], df = 6)) #copying the spline basis expansion from the real data set applications
  z = df$z
  H0 = df$H0
  ...
  write.csv(df_out, paste('result_data1/zap_', file, sep = ''))
}
...
for (file in files) {
  print(file)
  df = read.csv(paste('data2', file, sep = '/'))
  x = df[, grepl('x', colnames(df))]
  z = qnorm(pexp(df$z, 1)) #transformation to null-normal
  H0 = df$H0
  summary_FDP = rep(0, length(alphas))
  summary_power = rep(0, length(alphas))
  res_zap_asymp = zap_asymp(z, as.matrix(x), alpha = Inf) #Get mirror statistics
  ...
  df_out = data.frame(FDP = summary_FDP, power = summary_power, type = rep('zap', length(alphas)), alpha = alphas)
  write.csv(df_out, paste('result_data2/zap_', file, sep = ''))
}
