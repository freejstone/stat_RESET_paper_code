...
random.seed(10052025)
np.random.seed(10052025)

path = 'data'
alphas = [0.01, 0.05, 0.1, 0.2]
files = os.listdir(path)
files = [f for f in files if 'sim2' in f]
count = 0
for file in files:
    print(file)
    ...
    power_all = []
    for alpha in alphas:    
        res = md.adafdr_test(pvals, x, alpha=alpha, fast_mode=False, random_state = 10052025 + count)
        t = res['threshold']
        D = np.sum(pvals<=t)
        FD = np.sum((pvals<=t)&(~h))
    ...
    df_out['type'] = 'adafdr'
    df_out.to_csv("result_data/" + "adafdr_" + file, header=True, index=False, sep='\t')
