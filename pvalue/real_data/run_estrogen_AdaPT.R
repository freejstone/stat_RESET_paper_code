...
for (i in 1:2){
  ...
  for (j in 1:length(alphas)) {
    alpha = alphas[j]
    df_final = rbind(df_final, list(alpha = alpha, type = 'AdaPT', power = out$nrejs[j], time = difftime(end, start, units='mins'), seed = 24062024, order = i))
  }
  
  write.csv(df_final, paste('results/', 'estrogen_adapt.csv', sep = ''))
}